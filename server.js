...
var loggedIn = false;
var url = "mongodb://admin:password@ds119406.mlab.com:19406/jenny"
var db
...
// Posts the administrator message
app.post('/api/messages', (req, res) => {

  //For when the user doesn't specify a content type header
  ...
  db.collection('messages').save(data, (err, result) => {
    if(err) return console.log(err)
    console.log('message sent to end users')
  })
})
// Deletes an admin message from the database
app.delete('/api/messages/:id', (req, res) => {
  db.collection('messages').deleteOne({"_id": ObjectId(req.params.id)}, (err, result) => {
    if(err) return console.log(err)
